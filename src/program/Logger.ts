...
const getLoggerEntry =
    ...
    ): L.LoggerTask<LogEntry> =>
    (entry) =>
        T.fromIO(C.log(withColor(showEntry.show(entry))));

const debugLogger = L.filter(
    getLoggerEntry(chalk.cyan),
    (e) => e.level === "DEBUG"
);

const errorLogger = L.filter(
    getLoggerEntry(chalk.bold.red),
    (e) => e.level === "ERROR"
);

const infoLogger = L.filter(
    getLoggerEntry(chalk.bold.magenta),
    (e) => e.level === "INFO"
);

const mainLogger = pipe(
    [debugLogger, errorLogger, infoLogger],
    M.fold(L.getMonoid<LogEntry>())
);
...
export const showEntry: S.Show<LogEntry> = {
    show: ({ message, date, level }) =>
        `${showDate.show(date)} | ${level} | ${message}`,
};
...
export const Logger: Logger = {
    debug: (message) =>
        pipe(TE.fromTask<void, Error>(debug(message)), TE.mapLeft(toErrorMsg)),
    error: (message) =>
        pipe(TE.fromTask<void, Error>(error(message)), TE.mapLeft(toErrorMsg)),
    info: (message) =>
        pipe(TE.fromTask<void, Error>(info(message)), TE.mapLeft(toErrorMsg)),
};
