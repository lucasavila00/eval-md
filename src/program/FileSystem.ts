// from https://github.com/gcanti/docs-ts/blob/master/src/FileSystem.ts
 ...
 * @since 0.6.0
 */
export const exists: (path: string) => TE.TaskEither<Error, boolean> =
    TE.taskify<string, Error, boolean>(fs.pathExists);
...
export const FileSystem: FileSystem = {
    ...
    writeFile: (path, content) =>
        pipe(
            ...
            TE.mapLeft(toErrorMsg)
        ),
    exists: flow(exists, TE.mapLeft(toErrorMsg)),
    remove: (pattern) => pipe(remove(pattern, {}), TE.mapLeft(toErrorMsg)),
    search: (pattern, exclude) =>
        pipe(search(pattern, { ignore: exclude }), TE.mapLeft(toErrorMsg)),
};
